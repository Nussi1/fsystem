...
def add_file(args):
  ...
  hash_dir(path)
  shutil.copyfile(get_file_path(file_name), file_dir(path)+path)
  ...
  load_content.update({path : [getfilesize(file_name), get_time(file_name), {'filename': file_name}]})
...
def getfilesize(filename):
  return os.stat(get_file_path(filename)).st_size / 1024


def get_time(file_time):
  return time.ctime(os.path.getmtime(get_file_path(file_time)))
...
def copy_file_name(name_of_file):
...
def file_dir(arg):
  s = '/'.join(list(arg[:4]))
  return settings.PATH['path_obj'] + s + '/files/'
